...
use crate::{
    combo_box,
    consts::{
        BUTTON_BORDER, BUTTON_HEIGHT, BUTTON_SPACING, DIALOG_BUTTON_HEIGHT, DIALOG_BUTTON_WIDTH,
        GRID_BORDER, ICON_SIZE, MARGIN, SPACING, TABLE_HORIZONTAL_MARGIN,
    },
...
fn settings_editor() -> impl Widget<State> {
    Flex::column()
        ...
        .with_child(
            Flex::row()
                .with_child(Button::new("Layout").on_click(|_, state: &mut State, _| {
                    state.on_component_settings_tab = false;
                }))
                .with_child(
                    Button::new("Component").on_click(|_, state: &mut State, _| {
