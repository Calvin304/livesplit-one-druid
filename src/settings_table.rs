...
use crate::{
    combo_box,
    consts::{BUTTON_BORDER, GRID_BORDER},
...
use livesplit_core::{
    ...
    settings::{
        self, Alignment, Font, FontStretch, FontStyle, FontWeight, Gradient, ListGradient, Value,
    },
                    ...
                    ViewSwitcher::new(
                        ...
                        |_, row, _| match row.value {
                            ...
                            Value::Alignment(_) => Box::new(
                                combo_box::widget(&["Automatic", "Left", "Center"])
                                    .lens(Identity.map(
                                        |row: &SettingsRow| match &row.value {
                            ...
                            Value::LayoutDirection(_) => Box::new(
                                combo_box::widget(&["Vertical", "Horizontal"])
                                    .lens(Identity.map(
                                        |row: &SettingsRow| match &row.value {
...
fn column_update_trigger() -> impl Widget<SettingsRow> {
    combo_box::widget(&["On Starting Segment", "Contextual", "On Ending Segment"])
        .lens(Identity.map(
            |row: &SettingsRow| match &row.value {
...
fn gradient() -> impl Widget<SettingsRow> {
    Flex::column()
        .with_child(
            combo_box::widget(&["Transparent", "Plain", "Vertical", "Horizontal"])
                .lens(Identity.map(
                    |row: &SettingsRow| match &row.value {
                    ...
                    Flex::column()
                        ...
                        .with_child(
                            Flex::row()
                                ...
                                .with_flex_child(
                                    combo_box::widget(&["Normal", "Italic"])
                                        .lens(Identity.map(
                                            |row: &SettingsRow| match &row.value {
...
fn digits_format() -> impl Widget<SettingsRow> {
    combo_box::widget(&["1", "01", "0:01", "00:01", "0:00:01", "00:00:01"])
        .lens(Identity.map(
            |row: &SettingsRow| match &row.value {
...
fn accuracy() -> impl Widget<SettingsRow> {
    combo_box::widget(&["Seconds", "Tenths", "Hundredths", "Milliseconds"])
        .lens(Identity.map(
            |row: &SettingsRow| match &row.value {
            ...
            ViewSwitcher::new(
                ...
                |_, row, _| match row.value {
                    Value::OptionalTimingMethod(Some(_)) => {
                        Box::new(
                            combo_box::widget(&["Real Time", "Game Time"])
                                .lens(Identity.map(
                                    |row: &SettingsRow| match &row.value {
