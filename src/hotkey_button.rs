...
use crate::consts::GRID_BORDER;

#[derive(Clone, Copy)]
pub struct Hotkey(pub Option<KeyCode>);

impl fmt::Display for Hotkey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(key) = &self.0 {
            f.write_str(&key.resolve())
        } else {
            Ok(())
        }
    }
...
impl From<Code> for Hotkey {
    fn from(c: Code) -> Hotkey {
        Hotkey(Some(match c {
            ...
            // Code::Hiragana => KeyCode::Hiragana,
            // Code::Katakana => KeyCode::Katakana,
